...
class NNArch(nn.Module):
    def __init__(self, args):
        ...
        self.global_pooling_fc = nn.Linear(global_pooling_c * 2, global_pooling_c)
        ...
        self.pi_fc1 = nn.Linear(32 * in_x * in_y, args.pi_size)
    ...
    def forward(self, s):
        ...
        x = x.view(-1, INPUT_SIZE[0]+NN_CHANNELS*(NN_DEPTH//2), INPUT_SIZE[1] * INPUT_SIZE[2])
        y = torch.mean(x, dim=2)
        z, _ = torch.max(x, dim=2)
        s = s + self.global_pooling_fc(torch.cat([y, z], 1)).unsqueeze(-1).unsqueeze(-1)
