...
class EloInfo:
    ...
    def get_log10_odds_surprise_max_likelihood(
        self, p1: Player, p2: Player, g1: float, total: int
    ) -> float:
        ...
        if g1_frac != 0.0:
            ...
            log_prob = scipy.special.logsumexp(
                [logxf + math.log(1.0 - g1_frac), logxp1 + math.log(g1_frac)]
            )
        else:
        ...
        log_odds = log_prob - (
            math.log(-log_prob)
            if log_prob > -1e-10
            else math.log(1.0 - math.exp(log_prob))
        )
        return -signflip * max(0.0, -log_odds / math.log(10.0))

    def get_approx_log10_odds_surprise_bayes(
        self, p1: Player, p2: Player, g1: float, total: int
    ) -> float:
    ...
    def __str__(self) -> str:
        ...
        for player in self.players:
            lines.append(
                f"{player:20s}: {self.elo[player]:8.2f} +/- {self.elo_stderr[player]:5.2f}"
            )
        return "\n".join(lines)
...
def compute_elos(
    ...
    * All players *have* won and lost against other players, but there is nothing that anchors the Elo of the
    ...
    for i in range(max_iters):
        if verbose:
            print(
                f"Beginning iteration {i}, cur log likelihood {loglikelihood}, last elo change {last_elo_change}"
            )
...
class GameResultSummary:
    ...
    def add_game_record(self, record: GameRecord):
        ...
        if (record.player1, record.player2) not in self.results:
            self.results[(record.player1, record.player2)] = GameRecord(
                player1=record.player1, player2=record.player2
            )
        self.results[(record.player1, record.player2)].win += record.win
        self.results[(record.player1, record.player2)].loss += record.loss
        self._game_count += record.win + record.loss
    ...
    def print_elos(self):
        ...
        real_players = [
            player for player in elo_info.players if player != P1_ADVANTAGE_NAME
        ]
            ...
            for pla2 in real_players:
                if pla1 == pla2:
                    ...
                    pla1_pla2 = (
                        ...
                        else GameRecord(pla1, pla2)
                    )
                    pla2_pla1 = (
                        self.results[(pla2, pla1)]
                        if (pla2, pla1) in self.results
                        else GameRecord(pla2, pla1)
                    )
                    win = (
                        pla1_pla2.win
                        + pla2_pla1.loss
                    ...
                    surprise = elo_info.get_log10_odds_surprise_max_likelihood(
                        pla1, pla2, win, total
                    )
                    if total <= 0:
                        row.append("-")
...
if __name__ == "__main__":
    ...
    for i in range(record_count):
        p1, p2, score, total = records[i * 4 : i * 4 + 4]
        score = float(score)
        total = float(total)
        summary.add_game_record(GameRecord(p1, p2, win=score, loss=total - score))
