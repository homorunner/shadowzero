...
// XXHash is state of the art in 64-bit hash algorithms.
// If 64 bit is not enough, maybe we can switch to MetroHash for 128-bit
// support.
/* How to use:
    uint64_t myseed = 0;
    XXHash64 myhash(myseed);
    myhash.add(pointerToSomeBytes,     numberOfBytes);
    myhash.add(pointerToSomeMoreBytes, numberOfMoreBytes);
...
class XXHash64 {
 public:
  ...
  void add(const void* input, uint64_t length) {
    // no data ?
