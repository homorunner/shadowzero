...
class parser {
 public:
  ...
  // flag (boolean) accessors: return true if the flag appeared, otherwise
  // false.
  ...
  // returns a std::istream that can be used to convert a positional arg to a
  // typed value.
  string_stream operator()(size_t ind) const;

  // same as above, but with a default value in case the arg is missing (index
  // out of range).
  template <typename T>
  string_stream operator()(size_t ind, T&& def_val) const;

  // parameter accessors, give a name get an std::istream that can be used to
  // convert to a typed value. call .str() on result to get as string
  ...
  // If T only has an input stream operator, pass the string version of the type
  // as in "3" instead of 3.
  template <typename T>
...
inline void parser::parse(int argc, const char* const argv[], int mode /*= PREFER_FLAG_FOR_UNREG_OPTION*/) {
  ...
  for (auto i = 0u; i < args_.size(); ++i) {
    ...
    // any potential option will get as its value the next arg, unless that arg
    // is an option too in that case it will be determined a flag.
    ...
    // PREFER_FLAG_FOR_UNREG_OPTION: a non-registered 'name' is determined a
    // flag.
    //                               The following value (the next arg) will be
    //                               a free parameter.
    //
...
inline bool parser::is_number(std::string const& arg) const {
  // inefficient but simple way to determine if a string is a number (which can
  // start with a '-')
